 ...
 * @param isVisible     是否显示在左侧导航栏
 * @param functionCode  按钮code
 * @param functionSort  菜单顺序
 */
export function addFunction(functionName, parentId, functionUrl, functionType, iconStyle, isVisible, functionCode, functionSort) {
  const data = {
    functionName,
    parentId,
    functionUrl,
  ...
  return request({
    url: '/system/admin/sysFunction/save',
    method: 'post',
...
export function getFunctionById(id) {
  return request({
    url: '/system/admin/sysFunction/getById',
    method: 'get',
    params: {
      id
    }
 ...
 * @param functionSort  菜单顺序
 */
export function updateFunction(id, functionName, parentId, functionUrl, functionType, iconStyle, isVisible, functionCode, functionSort) {
  const data = {
    id,
    functionName,
    parentId,
  ...
  return request({
    url: '/system/admin/sysFunction/update',
    method: 'post',
    headers: { 'content-type': 'application/json;charset=utf-8' },
    data: data
  })
...
export function removeFunction(ids) {
  return request({
    url: '/system/admin/sysFunction/removeByIds',
    method: 'post',
...
export function treeFunction() {
  return request({
    url: '/system/admin/sysFunction/tree',
    method: 'get'
  })
}
