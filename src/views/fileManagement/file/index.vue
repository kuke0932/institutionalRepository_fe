      ...
      <el-col :span="18">
        <el-form>
          ...
          <div class="btn_box top">
            <el-button class="el-button--primary hidden" code="addFile" @click="goAddFile">添加文件</el-button>
            <el-button class="el-button--primary hidden" code="editFile" @click="goEditFile">修改文件</el-button>
            <el-button class="el-button--danger hidden" code="removeFile" @click="deleteFile">删除文件</el-button>
            <el-button class="el-button--primary hidden" code="copyFile" @click="copyFile">复制文件</el-button>
            <el-button class="el-button--primary hidden" code="moveFile" @click="moveFile">移动文件</el-button>
            <el-button class="el-button--primary hidden" code="setFilePermission" @click="setPermission">设置权限</el-button>
            <el-button class="el-button--primary hidden" code="viewFilePermission" @click="viewPermission">查看权限</el-button>
          </div>
          <el-dialog
              title="添加文件"
...
<script>
  import { treeFolderPermission } from '@/api/folder'
  import { listFilePermission, removeFile } from '@/api/file'
  import Add from './add'
  ...
  export default {
    ...
    methods: {
      fetchTreeData() {
        treeFolderPermission().then(resp => {
      ...
      fetchFileData(folderId, fileRealName, fileTitle, pageNum, pageSize) {
        ...
        if (this.selectedFolder.folderPrivate === 1) {
          allButtons && allButtons.forEach(b => {
            const type = b.getAttribute('data-type')
            if (type === 'folder') { // 显示文件夹权限
          ...
          const length = (this.selectedFolder.permissionList || []).length
          for (let i = 0; i < length; i++) {
            const e = this.selectedFolder.permissionList[i]
            if (e.parentId === 1) { // 不显示文件权限
              return
            }
        ...
        listFilePermission(folderId, fileRealName, fileTitle, pageNum, pageSize).then(resp => {
      ...
      rowClicked(val) {
        ...
        if (val.folderPrivate === 1) {
          ...
          allButtons && allButtons.forEach(b => {
            const type = b.getAttribute('data-type')
            if (type === 'file') { // 不隐藏文件夹的权限
              b.style.display = 'none'
            }
          })
          console.log(val.permisionList)
          const length = (val.permissionList || []).length
          for (let i = 0; i < length; i++) {
            const e = val.permissionList[i]
            if (e.parentId === 2) { // 不显示文件夹权限
              break
            }
            const buttons = document.querySelectorAll('button[code=\'' + e.filePermissionCode + '\']')
            buttons && buttons.forEach(b => {
        ...
        if (this.isCheckedMany()) {
          ...
          removeFile(ids).then(resp => {
            this.$message({
              ...
              onClose: () => {
                listFilePermission(this.selectedFolder.id, '', '', this.pageNum).then(resp => {
                  this.tableData = resp.data.data.rows
                  this.total = resp.data.data.total
                  this.pageNum = resp.data.data.pageNum
                })
      ...
      closeDialog(add) {
        listFilePermission(this.selectedFolder.id, '', '').then(resp => {
          this.tableData = resp.data.data.rows
          this.total = resp.data.data.total
          this.pageNum = resp.data.data.pageNum
        })
