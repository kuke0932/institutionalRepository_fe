<template>
  <div class="container">
    <el-form ref="folderForm" :model="folderForm" :rules="rules" label-width="160px">
      ...
      <el-form-item label="父文件夹" prop="parentFolder">
        <el-input readonly="readonly" v-model="parentFolder.label"></el-input>
      </el-form-item>
      <el-form-item label="是否继承父文件夹权限" prop="fileExtendParentPermission">
        <el-tooltip :content="folderForm.fileExtendParentPermission == 1 ? '是' : '否'" placement="top">
          <el-switch
            v-model="folderForm.fileExtendParentPermission"
            active-color="#13ce66"
      ...
      <el-form-item label="文件夹顺序" prop="folderSort">
        <el-input v-model="folderForm.folderSort"></el-input>
      </el-form-item>
      <el-form-item label="设置文件夹权限">
...
<script>
  ...
  import { listAddRole, listAddGroup, addFolder } from '@/api/folder'
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          ...
          folderSort: [
            { required: true, message: '请输入文件夹顺序', trigger: 'blur' },
    ...
    methods: {
      ...
      /* 查询未添加的角色列表 */
      searchUnRole() {
        this.fetchUnHadRoleData(this.searchRoleName, this.searchRoleValue, this.unRolePageNum, this.unRolePageSize)
      },
      /* 添加角色保存按钮 */
      addRole() {
        this.unRoleSelections.forEach(v => {
          this.allRoleData.push(v)
        })
      ...
      deleteRole() {
        ...
        this.roleSelections.forEach(v => {
          this.removeById(this.allRoleData, v.id)
        })
      },
      /* =======================添加角色结束================================ */
      /* =======================添加部门开始================================ */
      ...
      goAddDepartment() {
        const ids = []
        this.allDepartmentData.forEach(v => {
          ids.push(v.id)
        })
        setTimeout(() => {
      ...
      addDepartment() {
        ...
        this.allDepartmentData = this.unDepartmentSelections
      ...
      deleteDepartment() {
        ...
        this.departmentSelections.forEach(v => {
          this.removeById(this.allDepartmentData, v.id)
        })
      },
      /* =======================添加部门结束================================ */
      /* =======================添加群组开始================================ */
      ...
      deleteGroup() {
        ...
        this.groupSelections.forEach(v => {
          this.removeById(this.allGroupData, v.id)
        })
      },
      /* 获取群组用户信息 */
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            const sysFolder = {
              ...
              folderExtendParentPermission: this.folderForm.fileExtendParentPermission,
              folderSort: this.folderForm.folderSort
            }
            const roleList = this.getList(this.allRoleData, this.checkedPermission.role)
            let flag = false
            let hint = '角色为【'
            this.allRoleData.forEach(v => {
            ...
            const departmentList = this.getList(this.allDepartmentData, this.checkedPermission.department)
            hint = '部门为【'
            this.allDepartmentData.forEach(v => {
            ...
            const groupList = this.getList(this.allGroupData, this.checkedPermission.group)
            hint = '群组为【'
            this.allGroupData.forEach(v => {
            ...
            addFolder(sysFolder, roleList, departmentList, groupList).then(() => {
              this.$message({
                type: 'success',
                message: '添加成功',
                duration: 2000,
                onClose: () => {
                  this.clearData()
                  this.$emit('closeAddDialog', this.parentFolder.id)
                }
              })
            })
          } else {
      ...
      clearData() {
        ...
        this.folderForm.fileExtendParentPermission = '0'
        this.folderForm.folderSort = ''
        this.checkedPermission.id = null
        this.checkedPermission.permissionType = null
