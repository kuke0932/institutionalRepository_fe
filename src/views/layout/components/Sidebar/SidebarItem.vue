  ...
  <div class="menu-wrapper">
    <template v-for="(item, index) in routes" v-if="!item.hidden&&item.children">
      ...
      <router-link v-if="item.children.length===1 && !item.children[0].children"
                   :to="{path: item.path+'/'+item.children[0].path, query: (item.children[0].meta && item.children[0].meta.query) || {}}"
                   :key="item.children[0].fullPath">
        <el-menu-item :index="item.path+'/'+item.children[0].path+generatePathQuery(item.children[0].meta && item.children[0].meta.query)"
      ...
      <el-submenu v-else :index="item.name||item.path +generatePathQuery(item.query)" :key="item.fullPath">
        <template slot="title">
        ...
        <template v-for="(child, childIndex) in item.children" v-if="!child.hidden">
          ...
          <router-link v-else :to="{path: item.path+'/'+child.path, query: (child.meta && child.meta.query) || {}}" :key="child.fullPath">
            <el-menu-item :index="item.path+'/'+child.path+generatePathQuery(child.meta && child.meta.query)">
