...
/** Custom controller for the save dialog   保存对话框的自定义控制器*/
var SaveModelCtrl = ['$rootScope', '$scope', '$http', '$route', '$location',
  function ($rootScope, $scope, $http, $route, $location) {
    ...
    $scope.save = function (successCallback) {
      ...
      // Indicator spinner image 指示器转轮形象
      $scope.status = {
        loading: true
      };
      ...
      // Remove all forced stripable elements  移除所有强制的有条纹的元素
      var stripOutArray = jQuery(svgClone).find(".stripable-element-force");
      for (var i = stripOutArray.length - 1; i >= 0; i--) {
        stripOutArray[i].remove();
      }

      // Parse dom to string  将dom解析为字符串
      ...
      $http({
        ...
        .success(function (data, status, headers, config) {
          if (data.status !== 200) {
            ...
            // Fire event to all who is listening  对所有正在听的人的火灾事件
            var saveEvent = {
