...
var KisBpmFormPropertiesPopupCtrl = ['$scope', '$q', '$translate', '$timeout', function ($scope, $q, $translate, $timeout) {
  // Put json representing form properties on scope 将表示表单属性的json放到作用域中
  ...
  // Handler for when the value of the type dropdown changes  当类型下拉列表的值改变时的处理程序
  $scope.propertyTypeChanged = function () {
    // Check date. If date, show date pattern 检查日期。如果日期，显示日期模式
    ...
    // Check enum. If enum, show list of options 检查枚举。如果枚举，显示选项列表
  ...
  // Click handler for add button 单击添加按钮的handler
  var propertyIndex = 1;
  ...
  // Click handler for remove button 舔处理器为删除按钮
  ...
  // Click handler for up button 单击handler for up按钮
  ...
  // Click handler for down button 单击handler for down按钮
  ...
  // Click handler for remove button 单击“删除处理程序”按钮
  ...
  // Click handler for save button 单击“保存”按钮
  $scope.save = function () {
