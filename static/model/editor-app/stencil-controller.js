...
angular.module('activitiModeler').controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function($rootScope, $scope, $http, $modal, $timeout) {
    ...
    // Add reference to global header-config 添加对全局header-config的引用
    ...
    //  依赖于初始化编辑器的代码被包装在编辑器的承诺中
    $scope.editorFactory.promise.then(function() {
        /* Build stencil item list  建立模具项目列表 */
        // Build simple json representation of stencil set 构建简单的模板集json表示
        var stencilItemGroups = [];
        // Helper method: find a group in an array 助手方法:在数组中查找组
        ...
        // Helper method: add a new group to an array of groups 助手方法:向组数组中添加一个新组
        ...
        $http({
            ...
            // Check all received items  检查所有收到的物品
            for (var stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++) {
                ...
                if (!removed) {
                    ...
                    if (currentGroupName !== null && currentGroupName !== undefined && currentGroupName.length > 0) {
                        ...
                        // Add all child groups (if any)  添加所有子组(如果有)
        ...
        $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function(event) {
            ...
            if (shapes && shapes.length > 0) {
                ...
                // Gather properties of selected item 收集选定项目的属性
                ...
                for (var i = 0; i < properties.length; i++) {
                    ...
                    // 首先，我们检查是否有一个配置用于'键类型'，然后单独用于'类型'
        ...
        $scope.quickAddItem = function(newItemId) {
            $scope.safeApply(function() {
                ...
                if (shapes && shapes.length == 1) {
                    ...
                    if (!targetStencil) {
                        return;
                    } // Check if there can be a target shape  检查是否有目标形状
